...
$this->js	->prepend('/kernel/ctl/noty/themes/default.js')
			...
			->prepend('/kernel/ctl/fancybox2/jquery.fancybox.js')
			->prepend('/kernel/ctl/control/js/plugins/ui/jquery.collapsible.min.js')
			->prepend('/kernel/ctl/uniform/jquery.uniform.js')
			->prepend('/kernel/js/jquery/jquery-migrate.js')
			->prepend('/kernel/js/jquery/jquery.js')
			->set(1000, '/kernel/ctl/control/js/custom.js');

$this->css	->prepend('/kernel/ctl/fancybox2/jquery.fancybox.css')
			->prepend('/kernel/ctl/control/css/ui_custom.css')
			->prepend('/kernel/ctl/control/css/dataTable.css')
			->prepend('/kernel/ctl/control/css/reset.css')
			->set(1000, '/kernel/ctl/control/css/main.css')
		...
		<div id="topNav">
			<div class="fixed">
				<div class="wrapper">
					...
					<div class="userNav">
						<ul>
							<?php echo $this->user('id') ? '<li><a href="'.$this->url(array('ccontroller' => 'cnotify', 'caction' => 'index'), 'control').'"><img src="/kernel/ctl/control/images/icons/topnav/messages.png" alt="" /><span>'.$this->translate('control_notify').'</span>'.($notify ? '<span class="numberTop">'.$notify.'</span>' : '').'</a></li>' : '' ?>
							<li><a href="<?php echo $this->url(array('controller' => 'index', 'action' => 'index')) ?>"><img src="/kernel/ctl/control/images/icons/topnav/mainWebsite.png" alt="" /><span><?php echo $this->translate('control_goindex') ?></span></a></li>
							<li><?php echo $this->user('id') ? '<a href="'.$this->url(array('ccontroller' => 'cuser', 'caction' => 'logout'), 'control').'"><img src="/kernel/ctl/control/images/icons/topnav/logout.png" alt="" /><span>'.$this->translate('control_logout').'</span></a>' : '<a href="'.$this->url(array('ccontroller' => 'cuser', 'caction' => 'login'), 'control').'"><img src="/kernel/ctl/control/images/icons/topnav/logout.png" alt="" /><span>'.$this->translate('control_login').'</span></a>' ?></li>
						</ul>
					</div>
					<div class="fix"></div>
				</div>
