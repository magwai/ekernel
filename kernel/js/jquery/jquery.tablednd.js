    ...
    makeDraggable: function(table) {
        ...
        for (var i=0; i<rows.length; i++) {
            ...
            if (! nodrag) { //There is no NoDnD attribute on rows I want to drag
				var obj = jQuery(rows[i]);
				if (config.dragSelector) obj = obj.find(config.dragSelector);
                obj.mousedown(function(ev) {
					var dro = ev.target.tagName == "TD" ? this : $(this).parents('tr:first')[0];
                    //if (ev.target.tagName == "TD") {
                        jQuery.tableDnD.dragObject = dro;
                        jQuery.tableDnD.currentTable = table;
                        jQuery.tableDnD.mouseOffset = jQuery.tableDnD.getMouseOffset(dro, ev);
                        if (config.onDragStart) {
                            // Call the onDrop method if there is one
                            config.onDragStart(table, dro);
                        }
                        return false;
                    //}
