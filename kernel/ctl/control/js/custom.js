...
c.init = function(d) {
	...
	$('.dataTables_length select').change(function () {
		window.location = $(this).data('url').replace(/\/perpage\/\d+/gi, '/perpage/' + $(this).val());
	});
	
	$('.dataTables_length select').uniform();
	
	//$(".c-select"/*, input:checkbox, input:radio, input:file"*/).uniform();
	...
	$('.table tbody tr').click(function() {
		var input = $(this).find('.c-tabl-cb input');
		$('.table .c-tabl-cb input').not(input).prop('checked', false);
		input.prop('checked', true);
		c.table_checkbox(input, false);
	});
	$('.table tbody .c-tabl-cb').click(function(event) {
		event.stopPropagation();
		$(this).find('input').click();
		return true;
	});
	$('.table tbody .c-tabl-cb input').click(function(event) {
		event.stopPropagation();
		c.table_checkbox($(this), event.shiftKey);
		return true;
	});
	$('.table thead .c-tabl-cb input').click(function(event) {
		$('.table tbody .c-tabl-cb input').prop('checked', $(this).prop('checked'));
...
c.table_checkbox = function(o, is_shift) {
	var list = $('.table tbody .c-tabl-cb input');
	...
	if (!o.prop('checked')) $('.table thead .c-tabl-cb input').prop('checked', false);
	c.table_checkbox_highlight(o);
}

c.table_checkbox_highlight = function(o) {
	if ((o && o.prop('checked')) || !$('.table tbody td.highlighted').parents('tr:first').find('.c-tabl-cb input').prop('checked')) $('.table tbody td').removeClass('highlighted');
	if (o && o.prop('checked')) o.parents('tr:first').find('td').addClass('highlighted');
	if (!$('.table tbody td.highlighted').length) {
		var list = $('.table tbody .c-tabl-cb input:checked');
...
c.table_gen_url = function(url, key_id) {
	var ids = [];
	var id = null;
	$('.table tbody .c-tabl-cb input:checked').each(function() {
		var el = $(this).parents('tr:first').data('id');
		if ($(this).parents('td:first').hasClass('highlighted')) id = el;
		ids.push(el);
	});
	if (ids.length == 1) ids = [];
